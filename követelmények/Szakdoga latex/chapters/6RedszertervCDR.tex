...
A jelen fejezetben bemutatjuk a CDR adatfeldolgozó rendszer tervezési döntéseit: milyen technológiai alternatívákat mérlegeltünk, és az adott korlátok mellett miért az alábbi eszközöket választottuk. A rendszer célja a YJMob100K adathalom (5.\ fejezet) feldolgozása, a 4.\ fejezetben ismertetett mobilitási metrikák kiszámítása, valamint a származtatott mutatók GIS alapú megjelenítése.
...
Az alábbiakban áttekintjük az egyes funkcióterületek mérlegelt alternatíváit, és megindokoljuk a végső választást.
...
Az ETL logikát közvetlenül Python szkriptekbe szerveztük, modul-szintű felelősségmegosztással. Egy-egy szkript egy-egy pipeline-fázist valósít meg, és a parancssorból futtatható. A köztes eredményeket Parquet formátumban is exportáljuk, ami hatékony oszlopos tárolást biztosít és biztonsági mentésként is szolgál.
...
\begin{enumerate}
    ...
    \item \textbf{Perzisztens tároló (DuckDB).} A betöltött adatok DuckDB táblákba kerülnek, amelyek oszlopos formátumban, tömörítve tárolódnak egyetlen fájlban (\texttt{mobility.duckdb}). Ez egyben a Data Warehouse szerepet is betölti, mivel a dimenziós táblákat is itt hozzuk létre.

    \item \textbf{Analitikai réteg.} A mobilitási metrikákat SQL CTE-láncokon (Common Table Expression) keresztül számítjuk ki a DuckDB-ben, majd az eredményeket Parquet fájlokba is exportáljuk. Az anomáliadetekciót a scikit-learn végzi Python-ban, a DuckDB-ből kiolvasott feature mátrixon.
...
Az adatbázis logikai modellje csillagsémára épül, amelyet a 3.\ fejezetben bemutatott általános CDR adatmodellből vezettünk le, a YJMob100K sajátosságaihoz igazítva.
...
Az YJMob100K 200$\times$200-as rácsa önmagában nem hordoz földrajzi információt. A rácsot az EPSG:2449 (Japan Plane Rectangular CS~VII) vetületi koordináta-rendszerbe helyeztük el a \texttt{reverse-engineering-YJMob100K-grid} projekt \cite{pinter2024revealing} eredményei alapján; a vetületi koordináták WGS84-re való átváltását a pyproj \texttt{Transformer} osztálya végzi a \texttt{geo.py} modulban. A rácsparaméterek (origó, anizotrop cellaméretek, tengelykonvenció) és a tényleges implementáció részleteit a 8.\ fejezet tárgyalja.

\subsection{Kockázatok és korlátok}

A rendszerterv megvalósítása során a következő korlátokkal számoltunk:
\begin{itemize}
    \item \textbf{Egyetlen gépes feldolgozás:} Elosztott cluster hiányában a DuckDB in-process motorjára támaszkodunk. A 141~millió sor kezelése 16~GB RAM mellett megoldható, de a párhuzamosítás a CPU magok számára korlátozódik.
    ...
    \item \textbf{Georeferálás bizonytalansága:} A rácsparamétereket inverz mérnöki módszerrel határoztuk meg; az origó pontos helye néhány száz méteres bizonytalansággal terhelt.
    \item \textbf{Anomáliadetekció validálása:} Felügyelet nélküli módszert alkalmazunk (Isolation Forest), amelynek kiértékelése címkézett ground truth adat hiányában korlátozott. Az eredményeket a vészhelyzeti időszak ismert kontextusa alapján értékeljük.
\end{itemize}
